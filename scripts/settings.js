document.addEventListener("DOMContentLoaded", function () {
  ...
  successMessage.style.color = "#0695b0";
  ...
  function updateMutedWordsDisplay() {
    ...
    muteCount.textContent = `${mutedWords.length} word${
      mutedWords.length !== 1 ? "s" : ""
    }`;
  ...
  function updateAddButtonState() {
    ...
    const isValid =
      word && !mutedWords.includes(word) && mutedWords.length < 20;
  ...
  chrome.storage.sync.get(
    ...
    function (data) {
      if (misleadingToggle)
        misleadingToggle.checked = data.filterMisleading ?? true;
      if (harassmentToggle)
        harassmentToggle.checked = data.filterHarassment ?? true;
      if (superficialityToggle)
        superficialityToggle.checked = data.filterSuperficiality ?? true;
      if (loweffortToggle)
        loweffortToggle.checked = data.filterLowEffort ?? true;
      if (intrusiveadsToggle)
        intrusiveadsToggle.checked = data.filterIntrusiveAds ?? true;
    }
  );
  ...
  function saveFilters() {
    chrome.storage.sync.set({
      ...
      filterSuperficiality: superficialityToggle
        ? superficialityToggle.checked
        : true,
      filterLowEffort: loweffortToggle ? loweffortToggle.checked : true,
      filterIntrusiveAds: intrusiveadsToggle
        ? intrusiveadsToggle.checked
        : true,
    });
  }

  if (misleadingToggle)
    misleadingToggle.addEventListener("change", saveFilters);
  if (harassmentToggle)
    harassmentToggle.addEventListener("change", saveFilters);
  if (superficialityToggle)
    superficialityToggle.addEventListener("change", saveFilters);
  if (loweffortToggle) loweffortToggle.addEventListener("change", saveFilters);
  if (intrusiveadsToggle)
    intrusiveadsToggle.addEventListener("change", saveFilters);
});
